...
public class Player {
	...
	class RankComparator implements Comparator<String> {
		public int compare(String arg0, String arg1) {
			String suit1 = arg0.split(" ")[2];
			String suit2 = arg1.split(" ")[2];
			if (suit1.compareTo(suit2) < 0) {
				return -1;
			} else if (suit1.compareTo(suit2) > 0) {
				return 1;
			} else {
				int number1 = Integer.parseInt(mapToNumber(arg0.split(" ")[0]));
				int number2 = Integer.parseInt(mapToNumber(arg1.split(" ")[0]));
				if (number1 < number2) {
					return -1;
				} else {
					return 1;
	...
	public static List<String> sortByRank() {
		List<String> allCards = DeckOfCards.distributeCards();
		Collections.sort(allCards, x.new RankComparator());
		return allCards;
	}
